"use client";
import { ValidatedTripInfo } from "@/lib/ai-data-validator";
import { Calendar, Users, Wallet2 } from "lucide-react";
import {
  useScroll,
  useTransform,
  motion,
} from "motion/react";
import React, { useEffect, useRef, useState } from "react";

interface TimelineEntry {
  title: string;
  content: React.ReactNode;
}

export const Timeline = ({ data, tripData }: { data: TimelineEntry[], tripData: ValidatedTripInfo }) => {
  const ref = useRef<HTMLDivElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const [height, setHeight] = useState(0);

  useEffect(() => {
    if (ref.current) {
      const rect = ref.current.getBoundingClientRect();
      setHeight(rect.height);
    }
  }, [ref]);

  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start 10%", "end 50%"],
  });

  const heightTransform = useTransform(scrollYProgress, [0, 1], [0, height]);
  const opacityTransform = useTransform(scrollYProgress, [0, 0.1], [0, 1]);

  // Defensive data validation
  if (!tripData || !data || data.length === 0) {
    return (
      <div className="w-full bg-white dark:bg-neutral-950 font-sans md:px-10 min-h-[50vh] flex items-center justify-center">
        <div className="text-center text-gray-500">
          <p>Trip details not available</p>
        </div>
      </div>
    );
  }

  return (
    <div
      className="w-full bg-white dark:bg-neutral-950 font-sans md:px-10"
      ref={containerRef}
    >
      <div className="max-w-7xl mx-auto py-7 px-4 md:px-8 lg:px-10">
        <h2 className="text-lg md:text-4xl mb-4 text-black dark:text-white max-w-4xl">
          Your Trip Itinerary from <strong className="text-primary">{tripData?.origin || 'Unknown'}</strong> to <strong className="text-primary">{tripData?.destination || 'Unknown'}</strong> is Ready
        </h2>
        <div className="flex gap-5 items-center flex-wrap">
          {tripData?.duration && (
            <div className="flex gap-2 items-center">
              <Calendar />
              <h2>{tripData.duration}</h2>
            </div>
          )}
          {tripData?.budget && (
            <div className="flex gap-2 items-center">
              <Wallet2 />
              <h2>{tripData.budget}</h2>
            </div>
          )}
          {tripData?.group_size && (
            <div className="flex gap-2 items-center">
              <Users />
              <h2>{tripData.group_size}</h2>
            </div>
          )}
        </div>
      </div>

      <div ref={ref} className="relative max-w-7xl mx-auto pb-20 min-h-[40vh]">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex justify-start pt-10 md:pt-10 md:gap-10"
          >
            <div className="lg:sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-[40%] mb-4 lg:mb-0">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-2" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-20 md:text-3xl font-bold text-neutral-500 dark:text-neutral-500 ">
                {item?.title || `Day ${index + 1}`}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-xl mb-4 text-left font-bold text-neutral-500 dark:text-neutral-500">
                {item?.title || `Day ${index + 1}`}
              </h3>
              <div className="min-h-[100px]">
                {item?.content || (
                  <div className="text-gray-500 text-center py-8">
                    <p>No activities scheduled for this day</p>
                  </div>
                )}
              </div>
            </div>
          </div>
        ))}
        <div
          style={{
            height: Math.max(height, 200) + "px",
          }}
          className="absolute md:left-8 left-8 top-0 overflow-hidden w-0.5 bg-[linear-gradient(to_bottom,var(--tw-gradient-stops))] from-transparent from-0% via-neutral-200 dark:via-neutral-700 to-transparent to-99%  mask-[linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)] "
        >
          <motion.div
            style={{
              height: heightTransform,
              opacity: opacityTransform,
            }}
            className="absolute inset-x-0 top-0  w-0.5 bg-linear-to-t from-purple-500 via-blue-500 to-transparent from-0% via-10% rounded-full"
          />
        </div>
      </div>
    </div>
  );
};